...
const extractSubtitles = (mkvPath, outputDir) => new Promise((resolve, reject) => {
	...
	console.log(`Searching for Romanian internal subtitle in ${mkvPath}`)
	...
	subs.once('tracks', tracks_ => {
		...
		if (roTracks.length === 0) {
			console.log('No Romanian internal subtitles found. Aborting...');
		}
		roTracks.forEach(track => {
			console.log(`Found Romanian internal subtitle in ${name} on track ${track.number}. Waiting for SUBRIP...`)
			const language = 'rum';
	...
	subs.on('finish', () => {
		...
		tracks.forEach((track, i) => {
			...
			console.log(`File saved successfully in ${track.file.path}`);
		})
		resolve(tracks_)
	})
